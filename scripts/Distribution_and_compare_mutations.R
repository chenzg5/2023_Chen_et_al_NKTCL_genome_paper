...
p + ggplot2::annotate("text", x=1.5,y = 2000,
                      label = paste("Brunner–Munzel",a[["p.value"]]),size=5)
...
#compare SVs between TP53-WT and TP53-MUT
library(tidyverse)
library(reshape2)
library(ggpubr) 
library(brunnermunzel)
SV_num <- read_excel("SV_boxplot.xlsx")
p=ggboxplot(SV_num, x = "TP53", y = "SV",
            xlab = "TP53 status",ylab = "The number of SVs",
            bxp.errorbar=T,color = "TP53", palette = "jco",
            #label.select=list(criteria = "y < 10"),
            add = "jitter")
#  Add p-value
#p + stat_compare_means()
b=brunnermunzel.test(SV ~ TP53, data = SV_num)
p + ggplot2::annotate("text", x=1.5,y = 600,
                      label = paste("Brunner–Munzel",b[["p.value"]]),size=5)
